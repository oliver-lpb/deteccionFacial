...
export class ImagenesService {
  ...
  cargarImagenesFirebase(imagen:FileItems, imagesData:ImagenesModel){
    ...
    const storeRef = ref(storage, `${this.CARPETA_IMAGENES}/${imagenTrim.replace(/ /g, '')}`);
    const uploadTask = uploadBytesResumable(storeRef,item.archivo);
    ...
    uploadTask.on('state_changed', (snapshot) =>{
      ...
      if(this.progress >+ 100){
        Swal.fire(
          'Exito', 'Se elimino correctamente', 'success'
        );
      }
